<template>
  <div class="add-smoothie card p-4 mt-3">
    <h4 class="pb-4 center-align">Edit {{ title }} Recipe</h4>
   <form @submit.prevent="submitSmoothie">
    ...
    <div class="field center-align">
      <button type="submit" class="btn pink mt-4 block">Edit Smoothie</button>
    </div>
   </form>
  </div>
</template>
...
export default {
  ...
  created(){
      db.collection('smoothies').where('slug', '==', this.$route.params.slug).get().then(snapshot => {
